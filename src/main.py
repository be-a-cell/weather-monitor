...
STATION_NAMES = {
    "C0T9D0": "彭佳嶼", "C0TA40": "雙溪", "C0TA50": "吉慶",
    "C0Z220": "富貴角", "C0Z230": "三芝", "C0Z310": "鼻頭角"
}
STATION_COLORS = {
    "C0TA40": "#FF5809", "C0TA50": "#A23400", "C0Z310": "#1F4E79",
    "C0T9D0": "#5B9BD5", "C0Z220": "#70AD47", "C0Z230": "#3399FF"
}
...
if chosen_font_prop is None:
    ...
    chosen_font_prop = font_manager.FontProperties(family=["Noto Sans CJK TC", "DejaVu Sans"])
        ...
        for i in range(total_days):
            ...
            for stn_id in STATIONS:
                ...
                try:
                    ...
                    station_list = res_data.get("data", []) if isinstance(res_data, dict) else []
                    if station_list and isinstance(station_list, list):
                        ...
                        for row in dts_list:
                            ...
                            temp = temp_obj.get("Instantaneous") if isinstance(temp_obj, dict) else temp_obj
                            rain_obj = row.get("Precipitation", {})
                            rain = rain_obj.get("Accumulation") if isinstance(rain_obj, dict) else rain_obj
            ...
            if daily_records:
                ...
                print(f"✅ {date_str} 成功存入 {len(daily_records)} 筆數據至 SQLite")
...
def generate_chart():
    ...
    plot_df["StationLabel"] = plot_df["station_id"] + " " + plot_df["station_name"]

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 10), sharex=True)
    station_labels = plot_df["StationLabel"].unique()

    total_days = (plot_df["datetime"].max() - plot_df["datetime"].min()).total_seconds() / 86400
    ...
    for label in station_labels:
        ...
        ax1.plot(
            group["datetime"], group["temperature"],
            marker="o", markersize=1.5, linewidth=1, label=label,
    ...
    for label in station_labels:
        ...
        if not rain_positive.empty:
            ax2.bar(
                rain_positive["datetime"], rain_positive["rainfall"],
                width=dynamic_bar_width, alpha=0.6, label=label,
                color=STATION_COLORS.get(s_id, "#333333")
            )
        else:
            ax2.plot(group["datetime"].iloc[:1], group["rainfall"].iloc[:1], alpha=0, label=label, color=STATION_COLORS.get(s_id, "#333333"))
    ...
    if chosen_font_prop:
        ...
        for text in leg1.get_texts() + leg2.get_texts(): text.set_fontproperties(chosen_font_prop)
        for ax in [ax1, ax2]:
